...
阅读时最好具备(Go、Java、C++)其中一门语言。一些少许的数据库知识：需要 了解（B+树|B树），事务的基本特性。
...
它的原理是在磁盘开辟一块空间作为虚拟内存，里面存储着虚拟内存地址与物理地址的映射，内存会加载这一小块虚拟地址，用户想读取整个文件时操作系统会给假的内存地址给用户，等真正去使用时再把磁盘数据加载到内存，用虚拟地址转换真正内存地址。最终达到的效果就是文件不需要自己读取文件到内存，而是由操作系统去完成，操作文件跟操作内存一样。
```go
//bolt_windows.go
